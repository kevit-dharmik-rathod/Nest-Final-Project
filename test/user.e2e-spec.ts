...
import { Department, departmentSchema } from "../src/modules/department/Schemas/dept.schema";
import { User, userSchema } from "../src/modules/user/Schemas/user.schema";
import { Test, TestingModule } from "@nestjs/testing";
import { MongooseModule, getModelToken } from "@nestjs/mongoose";
import { Attendance, attendanceSchema } from "../src/modules/attendance/Schemas/attendance.schema";
import { Student, studentSchema } from "../src/modules/student/Schemas/student.schema";
import { AppModule } from "../src/app.module";
import { AttendanceService } from "../src/modules/attendance/attendance.service";
import { StudentService } from "../src/modules/student/student.service";
import { Admin, depOne, depTwo, staffOne, staffTwo } from "../testStubs/testing.stubs";
...
describe('AppController (e2e)', () => {
    ...
    describe('login in route for admin ', () => {
        it('post router for admin login', async () => {
            const user = await request(app.getHttpServer()).post('/user/login').send({
    ...
    describe('add  user', () => {
        it('should add new staff member ', async () => {
            ...
            await request(app.getHttpServer()).post('/user/add').send(staffOne).set('Authorization', `Bearer ${token}`).expect(201);
        });
    })
    describe('add  user', () => {
        it('should add new staff member ', async () => {
            await request(app.getHttpServer()).post('/user/add').send(staffTwo).set('Authorization', `Bearer ${token}`).expect(201);
        });
    })
    ...
    describe('add department', () => {
        it('should create a new department', async () => {
            await request(app.getHttpServer()).post('/department/add').send(depOne).set('Authorization', `Bearer ${token}`).expect(201);
        });
    });
    describe('add department', () => {
        it('should create a new department', async () => {
            await request(app.getHttpServer()).post('/department/add').send(depTwo).set('Authorization', `Bearer ${token}`).expect(201);
        });
    });
    ...
    describe('get department by id', () => {
        it('should return department', async () => {
            const result = await request(app.getHttpServer()).get('/department/getall').set('Authorization', `Bearer ${token}`).expect(200);
            const dep = await request(app.getHttpServer()).get(`/department/${result.body[0]._id}`).set('Authorization', `Bearer ${token}`).expect(200);
        })
    })
    ...
    describe('add student by admin ', () => {
        it('should be able to add a student', async () => {
            const result = await request(app.getHttpServer()).get('/department/getall').set('Authorization', `Bearer ${token}`).expect(200);
            const student = await request(app.getHttpServer()).post('/student/add').send({
                name: 'jeet',
    ...
    describe('change student department', () => {
        it('should able to change department of the student', async () => {
            ...
            console.log('student id in update department', student._id);
            console.log('student in test update ', student);
            await request(app.getHttpServer()).patch(`/student/update/admin/${student._id}`).send({
                department: result.body[1]._id
    ...
    describe('delete user ', () => {
        it('should delete the user ', async () => {
            ...
            await request(app.getHttpServer()).delete(`/user/delete/${temp.id}`).set('Authorization', `Bearer ${token}`).expect(200);
        })
    })
    describe('get all users', () => {
        it('should return all the users', async () => {
            await request(app.getHttpServer()).get('/user/getAll').set('Authorization', `Bearer ${token}`).expect(200);
        })
    });

    describe('read my profile ', () => {
        it('should return login user profile', async () => {
            const result = await request(app.getHttpServer()).get('/user/whoami').set('Authorization', `Bearer ${token}`).expect(200);
    ...
    describe('admin updated own profile if it is login', () => {
        it('should be able to update profile', async () => {
            const result = await request(app.getHttpServer()).patch('/user/updateOwn').send({
    ...
    describe("user by it's id ", () => {
        it('should return user ', async () => {
            ...
            const result = await request(app.getHttpServer()).get(`/user/${temp.id}`).set('Authorization', `Bearer ${token}`).expect(200);
        });
    })
    describe('logout user ', () => {
        it('should log out user and remove the auth token', async () => {
            const temp = await userModel.findOne({ email: 'brook@gmail.com' });
            token = temp.authToken;
            const result = await request(app.getHttpServer()).post('/user/logout').set('Authorization', `Bearer ${token}`).expect(200);
    ...
    describe('should get 403 while getting all users because above login as staff', () => {
        it('should return all the users', async () => {
            const temp = await userModel.findOne({ email: 'staff1@gmail.com' });
            token = temp.authToken;
            await request(app.getHttpServer()).get('/user/getAll').set('Authorization', `Bearer ${token}`).expect(403);
        })
    });
    ...
    describe('login as a staff so only allow change to it password', () => {
        it('should be change only password', async () => {
            const temp = await userModel.findOne({ email: 'staff1@gmail.com' });
            await request(app.getHttpServer()).patch('/user/updateStaffItsProfile').send({
                password: '1234',
            }).set('Authorization', `Bearer ${temp.authToken}`).expect(200);
        })
    });
